...
Đây là source một Discord bot (`comatnhi`) viết bằng discord.py, music engine dùng yt-dlp + FFmpeg. Mục tiêu: refactor để **dễ deploy lên server mới**. Đọc kỹ `main.py`, `utils.py` và toàn bộ `cogs/` trước khi sửa. Đừng đổi logic tính năng, chỉ làm sạch + tách config + viết tài liệu.
...
Làm toàn bộ ngay trong thư mục local này, không cần push đi đâu.
